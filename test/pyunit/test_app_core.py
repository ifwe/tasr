...
class TestTASRCoreApp(TASRTestCase):
    ...
    def test_matched_subjects(self):
        '''POST /tasr/collection/subjects/match - gets subjects with metadata
        that matches the JSON filter posted with the request.  An empty JSON
        filter matches all subjects.'''
        # reg two vers for target subject and one for an alt subject
        self.register_subject(self.event_type)
        alt_subject_name = 'bob'
        self.register_subject(alt_subject_name)
        ...
        # now define a filter and get the matched subjects (none, so far)
        f_dict = {'bob': 'true'}
        matched_url = "%s/collection/subjects/match" % self.url_prefix
        resp = self.tasr_app.request(matched_url, method='POST',
                                     content_type=self.content_type,
                                     body=json.dumps(f_dict))
        self.abort_diff_status(resp, 200)
        meta_dict = SubjectHeaderBot.extract_metadata(resp)
        # we should have no GroupMetadata objects
        for sub_name in [self.event_type, alt_subject_name]:
        ...
        # now get the MATCHED subjects (should be one)
        resp = self.tasr_app.request(matched_url, method='POST',
                                     content_type=self.content_type,
                                     body=json.dumps(f_dict))
        self.abort_diff_status(resp, 200)
