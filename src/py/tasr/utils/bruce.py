...
Please see: https://github.com/ifwe/bruce
...
class BruceMsgCreator(object):
    ...
    @staticmethod
    def get_max_topic_size():
        '''This is the maximum topic size allowed by Kafka.'''
        return (2 ** 15) - 1

    @staticmethod
    def get_max_msg_size():
        ''' This is an extremely loose upper bound, based on the maximum value
        that can be stored in a 32-bit signed integer field.  The actual
        maximum is a much smaller value: the maximum UNIX domain datagram size
        supported by the operating system, which has been observed to be 212959
        bytes on a CentOS 7 x86_64 system.
        '''
        return (2 ** 31) - 1
    ...
    @staticmethod
    def create_any_partition_msg(topic, timestamp, key_bytes, value_bytes):
        '''Creates an ANY_PARTITION type message.'''
        topic_bytes = bytes(topic)

        if len(topic_bytes) > BruceMsgCreator.get_max_topic_size():
            raise BruceTopicTooLarge()

        msg_size = BruceMsgCreator.ANY_PARTITION_FIXED_BYTES + \
                len(topic_bytes) + len(key_bytes) + len(value_bytes)

        if msg_size > BruceMsgCreator.get_max_msg_size():
        ...
        buf.write(struct.pack('>ihhhh', msg_size,
                              ...
                              BruceMsgCreator.ANY_PARTITION_API_VERSION,
                              flags, len(topic_bytes)))
    ...
    @staticmethod
    def create_partition_key_msg(partition_key, topic, timestamp,
                                 key_bytes, value_bytes):
        '''Creates a message specifying a partition.'''
        topic_bytes = bytes(topic)
        ...
        buf.write(struct.pack('>ihhhih', msg_size,
                              ...
                              BruceMsgCreator.PARTITION_KEY_API_VERSION,
                              flags, partition_key, len(topic_bytes)))
        buf.write(topic_bytes)
        buf.write(struct.pack('>qi', timestamp, len(key_bytes)))
        buf.write(key_bytes)
        buf.write(struct.pack('>i', len(value_bytes)))
